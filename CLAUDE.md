...
The request pipeline flows as: `main.py → engine.py → (msg_parser.py + resolver.py + sheets.py + duration.py)`

- **main.py** — FastAPI app with `/slack/events` endpoint. Handles Slack signature verification, rate limiting, event deduplication, and channel filtering. Dispatches to `process_attendance` as a background task.
- **engine.py** — Core decision logic. Orchestrates parsing → name resolution → sheet lookup/write. Handles check-in (new row), check-out (update today or yesterday's row for overnight shifts), and checkout-only (N/A check-in).
- **msg_parser.py** — Regex-based message classifier. Returns a `ParseResult` dataclass with action (`checkin`/`checkout`/`skip`), optional time string, and Jibble metadata.
...
- **sheets.py** — `GoogleSheetsClient` wraps gspread. Sheet columns: A=Name, B=Date, C=Check-In, D=Check-Out, E=Duration. Has retry logic for Google API rate limits (429s).
- **duration.py** — Calculates time duration between check-in/check-out strings, handles overnight shifts.
